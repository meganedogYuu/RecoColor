...
export function hexToRgbObject(hex: string): RgbObject {
  ...
  // 強制的に6文字のhex値にする
  const sixCharacterHex: string =
    exclusionHex.length === 3
...
export function getClashingColorFrom(rgbColor: RgbColor): RgbColor {
  return new RgbColor({
    r: getClashing(rgbColor.r),
    g: getClashing(rgbColor.g),
    b: getClashing(rgbColor.b),
  })
 ...
 * @returns {{h: number; s: number; v: number}}
 */
export function addHueToHsvObject(
  hsv: HsvObject,
  addHue: number
...
export function hsvObjectsToSpecifiedType(
  hsvObjects: HsvObject[],
  type: ColorType,
  option: { type: string; decimal: number } = { type: 'round', decimal: 0 }
  ...
  switch (type) {
    ...
    case ColorType.Hex:
      return hsvObjects.map(hsv => hsvToRgbObject(hsv)).map(rgb => rgbObjectToHexString(rgb))
  }
  // ColorType.Rgb もしくはそれ以外の場合は Rgb として返す
  return hsvObjects.map(hsv => hsvToRgbObject(hsv)).map(rgb => highMathToRgbObject(rgb))
}
